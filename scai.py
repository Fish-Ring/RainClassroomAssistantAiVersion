...
def init_ai_client(config=None):
    ...
    if my_sk:  # 只有提供了API密钥才创建客户端
        client = OpenAI(api_key=my_sk, base_url=base_url)
    else:
        client = None
...
def get_answer(question_info):
    ...
    try:
            ...
            for option in options:
                if isinstance(option, dict) and "key" in option and "value" in option:
                    ...
                    formatted_options.append(
                        f"{chr(65+len(formatted_options))}.{option}"
                    )
                else:
                    formatted_options.append(
                        f"{chr(65+len(formatted_options))}.{str(option)}"
                    )
        ...
        # 添加调试信息，打印题目内容
        print(
            f"AI获取到的题目内容 - title: '{title}', options: {options}, problem_text: '{problem_text}'"
        )
        print(
            f"题目类型: {question_type}, 是否有选项: {bool(options)}, 是否有预设答案: {bool(blanks)}"
        )
        ...
        if blanks and question_type == 3:
            return (
                [blank["answers"][0] for blank in blanks]
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"题目：{problem_text} 请直接回答，不要解释。",
                    },
                ],
            ...
            if isinstance(ai_answers, list):
                ai_answers = [
                    answer for answer in ai_answers if answer and str(answer).strip()
                ]
            else:
                ai_answers = (
                    [ai_answers] if ai_answers and str(ai_answers).strip() else []
                )
            ...
            if ai_answers and any(
                str(answer).lower() in [word.lower() for word in confirmation_words]
