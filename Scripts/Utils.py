...
def show_ai_popup(text, title, config=None):
    # 根据配置显示AI消息弹窗
    if config and isinstance(config, dict):
        ai_config = config.get("ai_config", {})
        if ai_config.get("ai_popup", True):
...
def get_config_dir():
    ...
    dir_route = appdata_route + "\\RainClassroomAssistantAi"
    return dir_route
...
def show_ai_popup(message, title, config=None):
    """显示AI相关弹窗提示"""
    # 检查配置是否允许显示AI弹窗
    if config and config.get("ai_config", {}).get("show_popup", True):
        show_info(message, title)
    else:
        # 如果配置不允许或没有配置，只在日志中记录
        print(f"AI弹窗提示: {title} - {message}")


def show_info(message, title="提示"):
    """显示信息提示"""
    # 这里应该调用实际的GUI提示函数
    # 由于show_info函数可能在其他文件中有实现，这里提供一个简单的占位实现
    print(f"信息提示: {title} - {message}")