...
class Config_Ui(object):
    def setupUi(self, Dialog):
        ...
        self.model_input.setPlaceholderText("deepseek-ai/DeepSeek-V3.2-Exp")
        ...
        self.top_p_label = QtWidgets.QLabel("Top_p:")
        ...
        self.ai_answer_on.stateChanged.connect(self.enable_ai_config)
    ...
    def enable_answer_config(self):
        ...
        if self.answer_on.isChecked():
            ...
            self.ai_answer_on.setEnabled(True)
            self.enable_ai_config()
        else:
            self.when_answer_on.setEnabled(False)
    ...
    def save_config(self, dialog):
        ...
        if "ai_config" not in config:
            config["ai_config"] = {}
        config["ai_config"]["api_key"] = self.api_key_input.text()
        config["ai_config"]["base_url"] = self.base_url_input.text()
        config["ai_config"]["model"] = self.model_input.text()
